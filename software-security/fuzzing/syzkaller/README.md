...
- 修正 QEMU 网络接口命名问题：
  ...
  1. 若无错误，则跳过这一步
  2. 否则，在 `syzkaller/vm/qemu/qemu.go` 中，通过内核启动参数 `CmdLine` 添加 `net.ifnames=0`，强制使用传统网卡命名（`eth0`），以确保网络服务正常启动并分配 IP
...
- 更改 `linux` 目录下的 `.config` 文件，在最后面添加如下代码：

  ```ini
  # 内核代码覆盖率收集 (Kernel Coverage)
  CONFIG_KCOV=y
...
- 等待一段时间，编译完成后，就能在目录下看到`vmlinux` (kernel binary) 和 `bzImage` (packed kernel image)：
...
- 使用 `create-image.sh` 构建 `syzkaller` `qemu` 镜像：
...
- 确保内核能够`sshd`启动：
  ...
  1. 在虚拟机中，使用 `poweroff` 或 `shutdown -h now` 正常关机（`exit` 仅退出 `shell`，不一定关闭虚拟机）
  2. 在宿主机中，按 `Ctrl + A` 再按 `X` 关闭 `qemu`
...
- 之后可以通过 `ssh` 在另一个终端中连接到 `qemu` 实例。打开另一个终端：
